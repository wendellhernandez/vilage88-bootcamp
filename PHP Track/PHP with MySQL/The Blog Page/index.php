<?php
    ...
    $sql1 = "SELECT *, date_format(reviews.created_at, '%M-%d-%Y %h:%i:%s%p') AS review_date, reviews.id AS review_id FROM users INNER JOIN reviews ON users.id = reviews.user_id ORDER BY reviews.created_at DESC";
...
<html lang="en">
    <head>
        ...
        <title>My Account | The Blog Page</title>
...
<?php
    if(isset($_SESSION['id'])){
?>
                    <li><a href="#" class="nav_username">Welcome <?= $row['first_name'] . ' ' . $row['last_name'] ?>!</a></li>
...
?>
            <div>
                <span><?= $full_name ?> (<?= $review_date ?>)</span>
                <p><?= $content ?></p>
                <div class="reply_container">
<?php 
        $sql2 = "SELECT *, date_format(replies.created_at, '%M-%d-%Y %h:%i:%s%p') AS reply_date
        FROM users
        ...
        while($row2 = mysqli_fetch_assoc($result2)){
            $reply_date = $row2['reply_date'];
            $first_name = $row2['first_name'];
            $last_name = $row2['last_name'];
            $full_name = $first_name . ' ' . $last_name;
            $content = $row2['content'];
?>
                    <span><?= $full_name ?> (<?= $reply_date ?>)</span>
                    <p><?= $content ?></p>
<?php   }
        if(isset($_SESSION['id'])){
?>
